...
define('SESSION_IDLE_LIMIT', 30 * 60); // 30 min neaktivity
...
function requireAuth(): void {
  ...
  if (!empty($_SESSION['el_last_activity']) && (time() - $_SESSION['el_last_activity']) > SESSION_IDLE_LIMIT) {
    logout();
    header('Location: /prihlaseni?timeout=1');
    exit;
  }
  $_SESSION['el_last_activity'] = time();
}
...
function registerFailedLogin(): void {
  ...
  elLogSecurity("Failed login attempt from $ip (attempt {$all[$ip]['count']})");
}
